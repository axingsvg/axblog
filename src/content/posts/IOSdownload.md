---
title: IOS App Store 免越狱下载旧版应用
published: 2025-12-06
description: 无需越狱，应用自由升降级。
tags: [IOS]
category: 绿色软件
draft: false
---

# 软件介绍

无需越狱，应用任意历史版本一键下载，自由降级或安装经典版本，彻底告别强制更新，完全掌控你的应用选择权。

# 软件截图

![img](https://youke2.picui.cn/s1/2025/12/13/693ca811aefe8.png "Optional title")

# 更新日志

新增功能：

自动前往 iTunes Store（itms://），通过右键菜单设置开关，默认开启。

添加 “前往iTunes Store” 右键菜单选项。

解决 iTunes 搜索结果只显示博客单集的问题，用户可直接跳转至 APP 页面。

优化功能：

搜索框支持配置保存，自动记录上次选项。

# 使用方法

1、直接搜索方式

搜索应用，双击选择要下载的版本，在 iTunes 中下载即可。

2、复制应用链接方式

在 iTunes 下载按钮右侧下拉菜单中选择复制链接，双击选择要下载的版本，在 iTunes 中下载即可。

# iTunes 下载

64位：https://secure-appldnld.apple.com/itunes12/091-87819-20180912-69177170-B085-11E8-B6AB-C1D03409AD2A6/iTunes64Setup.exe

32位：https://secure-appldnld.apple.com/itunes12/091-87820-20180912-69177170-B085-11E8-B6AB-C1D03409AD2A5/iTunesSetup.exe

B站视频教程：https://www.bilibili.com/video/BV1VQ4y1M77t/?vd_source=d4b4b1cf5f15f21e52d020baef926c43

# 下载地址

蓝奏云：https://wwbfe.lanzouq.com/iaAkc3dht53g
